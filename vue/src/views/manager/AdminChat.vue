...
const loadConversations = async () => {
  try {
    const res = await request.get('/admin/chat/conversations', { params: { pageSize: 50 } })
    if (res.code === '200') {
...
const createConversation = async () => {
  try {
    const res = await request.post('/admin/chat/conversation', { title: '新对话' })
...
const deleteConversation = async (conversationId) => {
  try {
    ...
    const res = await request.delete(`/admin/chat/conversation/${conversationId}`)
    if (res.code === '200') {
...
const sendMessage = async () => {
  ...
  try {
    const response = await authenticatedFetch('/admin/chat/send', {
      method: 'POST',
      credentials: 'include',
      headers: {
